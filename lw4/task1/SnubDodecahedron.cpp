...
SnubDodecahedron::SnubDodecahedron(float size)
	: m_size(size)
{
	constexpr glm::vec4 defaultColor{ 0.5, 0.5, 0.5, 1 }; 
	for (unsigned index = 0; index < m_colorsCount; index++)
	{
		SetSideColor(index, defaultColor);
	}
}

void SnubDodecahedron::Draw() const
{
	static constexpr float vertices[60][3] = {
		{0,0,1.028031},
	...
	static const std::vector<std::vector<unsigned char>> faces{
		{0,1,2},
		{0,2,3},
		{0,3,4},
		{0,4,5},
	...
	static const std::vector<std::vector<unsigned char>> pentagonFaces {
		////{ 0,5,14,6,1 },
		{ 0,5,14 },
		{ 14, 6, 0 },
		{ 6, 1, 0 },
	...
	static const std::vector<std::vector<unsigned char>> lines {
		{42,30},
		{18,30},
		{34,22},
		{47,55},
	...
	glBegin(GL_TRIANGLES);
	{
		...
		{
			AddVertices(glm::value_ptr(m_sideColors[face]), faces.at(face), vertices);
		}
		for (size_t face = 0; face < pentagonFaces.size(); face = face + 3)
		{
			AddVertices(glm::value_ptr(m_pentagonsColor), pentagonFaces.at(face), vertices);
			AddVertices(glm::value_ptr(m_pentagonsColor), pentagonFaces.at(face+1), vertices);
			AddVertices(glm::value_ptr(m_pentagonsColor), pentagonFaces.at(face+2), vertices);
		}
	}
	glEnd(); 
	glBegin(GL_LINES);
	{
		for (size_t face = 0; face < lines.size(); ++face)
		{
			glColor4fv(glm::value_ptr(glm::vec4{0,0,0,0}));
			glm::vec<3, float> p0 = glm::make_vec3(vertices[lines[face][0]]);
			glm::vec<3, float> p1 = glm::make_vec3(vertices[lines[face][1]]);
...
void SnubDodecahedron::AddVertices(const GLfloat* color, const std::vector<unsigned char> facePoints, const float vertices[60][3]) const
{
	glColor4fv(color);

	glm::vec<3, float> p0 = glm::make_vec3(vertices[facePoints[0]]);
	glm::vec<3, float> p1 = glm::make_vec3(vertices[facePoints[1]]);
	glm::vec<3, float> p2 = glm::make_vec3(vertices[facePoints[2]]);
...
unsigned SnubDodecahedron::GetSideColorsCount()const
{
	return m_colorsCount;
}

void SnubDodecahedron::SetSideColor(unsigned index, const glm::vec4& color)
{
	if (index >= m_colorsCount)
	{
		throw std::exception("expected index less then max available color count");
	}
	m_sideColors[index] = color;
}

void SnubDodecahedron::SetSpecularColor(glm::vec4 color)
{
	m_specularColor = color;
}

void SnubDodecahedron::SetPentagonsColor(glm::vec4 color)
{
	m_pentagonsColor = color;
}

void SnubDodecahedron::SetShininess(float shininess)
{
