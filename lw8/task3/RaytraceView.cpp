...
void CRaytraceView::AddSomeSpheres()
{
	...
	auto shader = std::make_shared<CSimpleShader>(yellow);
...
void CRaytraceView::AddSomeConicCylinders()
{
	...
	AddConicCylinder(std::make_shared<CSimpleShader>(white), 2, 1, 1, transform);
	...
	AddConicCylinder(std::make_shared<CSimpleShader>(red), 1, 1, 0, coneTransform);
	...
	AddConicCylinder(std::make_shared<CSimpleShader>(green), 1, 0.5, 0.3, conicFrustumTransform);
}
...
void CRaytraceView::AddSomeCube()
{
	...
	CSimpleMaterial color;
	color.SetDiffuseColor(CVector4f(0, 1, 1, 1));
	color.SetAmbientColor(CVector4f(0.2, 0.2, 0.2, 1));
	color.SetSpecularColor(CVector4f(0.5, 0.5, 0.5, 1));

	AddTriangleMesh(std::make_shared<CSimpleShader>(color, m_cameraPos), std::make_shared<CTriangleMeshData>(std::move(shape.GetVertices()), shape.GetFaces()), transform);
}
...
void CRaytraceView::AddSomeTetrahedron()
{
	...
	CSimpleMaterial color;
	color.SetDiffuseColor(CVector4f(0.5f, 0.8f, 1, 1));
	color.SetAmbientColor(CVector4f(0.2, 0.2, 0.2, 1));
	color.SetSpecularColor(CVector4f(0.5, 0.5, 0.5, 1));
...
{
	CModelLoader loader;
	assert(loader.Load("assets/torus.3DS"));

	CMatrix4d transform;
	transform.Translate(-5, 2.5, -6.5);
	transform.Rotate(150, 0, 1, 0);
	transform.Scale(0.05, 0.05, 0.05);

	AddTriangleMesh(std::make_shared<CSimpleShader>(loader.GetMaterials().at(0), m_cameraPos), std::make_shared<CTriangleMeshData>(std::move(loader.GetVertices()), loader.GetFaces()), transform);
}

void CRaytraceView::AddSomeDodecahedron()
{
	CModelLoader loader;
	assert(loader.Load("assets/dodecohedron.3DS"));
	
	CMatrix4d transform;
	transform.Translate(-2, 1.5, -2);
	transform.Rotate(170, 0, 1, 0);

	AddTriangleMesh(std::make_shared<CSimpleShader>(loader.GetMaterials().at(0), m_cameraPos), std::make_shared<CTriangleMeshData>(std::move(loader.GetVertices()), loader.GetFaces()), transform);
}

void CRaytraceView::AddSomeIcosahedron()
{
	CIcosahedron shape;
	...
	CSimpleMaterial color;
	color.SetDiffuseColor(CVector4f(1, 0.2f, 0.8, 1));
	color.SetAmbientColor(CVector4f(0.2, 0.2, 0.2, 1));
	color.SetSpecularColor(CVector4f(0.5, 0.5, 0.5, 1));
...
void CRaytraceView::AddSomeOctahedron()
{
	COctahedron shape;

	CMatrix4d transform;
	transform.Translate(5, 1.3, -1);
	transform.Rotate(170, 0, 1, 0);
	CSimpleMaterial color;
	color.SetDiffuseColor(CVector4f(0.2, 0.2f, 0.8, 1));
	color.SetAmbientColor(CVector4f(0.2, 0.2, 0.2, 1));
	color.SetSpecularColor(CVector4f(0.5, 0.5, 0.5, 1));
...
CSceneObject& CRaytraceView::AddTriangleMesh(std::shared_ptr<IShader const> shader, std::shared_ptr<CTriangleMeshData const> pMeshData, CMatrix4d const& transform)
{
	return AddSceneObject(std::make_shared<CTriangleMesh>(std::move(pMeshData), transform), std::move(shader));
}
