...
CMyApplication::CMyApplication(const char* title, int width, int height)
	...
	, m_picture(
		"assets/Cottage_FREE.3DS",
		"assets/defender.3ds",
		"assets/EU43_7.3ds",
		"assets/uploads_files_4291760_slide.3ds",
		"assets/uploads_files_2309145_GROUND.3ds",
		"assets/xpander.3ds"
	)
	...
	, m_animate(false)
{
	...
	m_picture.SetRotationSpeed(30);
	m_picture.SetInclinationAngle(23.5);
}
...
void CMyApplication::DrawPicture()
{
...
void CMyApplication::OnIdle()
{
	...
	m_picture.Animate(static_cast<float>(m_animationController.GetTimeDelta() * 0.001));
	PostRedisplay();
	Sleep(10);
}
//почему в каких то случаях источник света вращается в каких не вращается как сделать чтобы источник был неподвижен относительно наблюдателя 
...
void CMyApplication::OnKeyboard(unsigned char key, int /*x*/, int /*y*/)
{
	...
	if (key == 'r')
	{
		m_animate = m_animate ^ true;
		m_picture.SetAnimation(m_animate);
	}
}