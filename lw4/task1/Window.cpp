...
Window::Window(int w, int h, const char* title)
	...
	, m_snubDodecahedron(CUBE_SIZE)
{
	unsigned maxColorsCount = m_snubDodecahedron.GetSideColorsCount();
	glm::vec4 nextColor;
	...
	for (unsigned index = 0; index < maxColorsCount; index++)
	{
		m_snubDodecahedron.SetSideColor(index, colors[index % colors.size()]);
	}

	m_snubDodecahedron.SetPentagonsColor({ 0.5, 0.2, 0.5, 1 });
	m_snubDodecahedron.SetSpecularColor({ 1, 1, 1, 1 });
	m_snubDodecahedron.SetShininess(4.0f);
}
